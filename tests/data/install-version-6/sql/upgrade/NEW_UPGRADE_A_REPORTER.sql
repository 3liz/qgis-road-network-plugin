...
-- FUNCTION: editing_session.build_road_cached_objects(text)

-- DROP FUNCTION IF EXISTS editing_session.build_road_cached_objects(text);

CREATE OR REPLACE FUNCTION editing_session.build_road_cached_objects(
	_road_code text)
    RETURNS boolean
    LANGUAGE 'plpgsql'
    COST 100
...
COMMENT ON FUNCTION editing_session.build_road_cached_objects(text)
