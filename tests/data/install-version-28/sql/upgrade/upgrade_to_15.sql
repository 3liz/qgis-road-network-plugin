...
COMMENT ON FUNCTION road_graph.update_table_references_from_geometries(_schema_name text, _table_name text, _road_codes text[])
...
CREATE OR REPLACE FUNCTION road_graph.get_merged_geom_from_table_and_ids(_schema_name text, _table_name text, _ids integer[]) RETURNS geometry
    LANGUAGE plpgsql
    AS $_$
DECLARE
    primary_key_field text;
