...
-- aa_before_record_insert_or_update()
CREATE FUNCTION road_graph.aa_before_record_insert_or_update() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- Trigger disabled by session variable
...
-- drop_editing_session(integer)
CREATE FUNCTION road_graph.drop_editing_session(_editing_session_id integer) RETURNS boolean
    LANGUAGE plpgsql SECURITY DEFINER
    AS $$
DECLARE
    editing_session_record record;
...
CREATE FUNCTION road_graph.import_data_from_template_tables(_source_schema text, _source_edges_table text, _source_markers_table text) RETURNS json
    LANGUAGE plpgsql
    AS $_$
DECLARE
    sql_text text;
...
CREATE FUNCTION road_graph.update_managed_objects_on_graph_change(_schema_name text, _table_name text, _road_codes text[]) RETURNS integer
    LANGUAGE plpgsql
    AS $_$
DECLARE
    sql_text text;
...
CREATE FUNCTION road_graph.update_table_geometries_from_references(_schema_name text, _table_name text, _road_codes text[]) RETURNS jsonb
    LANGUAGE plpgsql
    AS $_$
DECLARE
    road_code text;
...
CREATE FUNCTION road_graph.update_table_references_from_geometries(_schema_name text, _table_name text, _road_codes text[], _update_offset_and_side boolean DEFAULT true) RETURNS jsonb
    LANGUAGE plpgsql
    AS $_$
DECLARE
    road_code text;
