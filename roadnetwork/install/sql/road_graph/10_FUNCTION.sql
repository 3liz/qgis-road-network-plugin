...
BEGIN
    ...
    WITH node AS (
        SELECT road_graph.get_upper_roundabout_node_to_delete(
            _road_code
        ) AS id
    )
    SELECT INTO merge_edges_result
        road_graph.merge_edges(
            (SELECT id FROM road_graph.edges WHERE end_node = n.id),
            (SELECT id FROM road_graph.edges WHERE start_node = n.id)
        )
    FROM node AS n
    ;
...
BEGIN
    ...
    IF to_jsonb(NEW) ? 'geom' THEN
        ...
        WHERE unique_code = TG_TABLE_SCHEMA::text
        ;
        RESET search_path;
        ...
        IF TG_OP IN ('INSERT', 'DELETE') THEN 
            ...
            IF NOT ST_Within(object_geom, editing_session_geom) THEN
                RAISE EXCEPTION 'The object geometry must be strictly inside the editing session polygon';
            END IF;
        END IF;
    ...
    SELECT INTO new_logged_ids
        logged_ids
    FROM editing_sessions
    WHERE unique_code = TG_TABLE_SCHEMA 
    ;
